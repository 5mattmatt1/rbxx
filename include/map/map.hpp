...
class map : public std::map<std::optional<Key>, std::optional<T>>
{
public:
    ...
    T default_value;
    std::function<T(map<Key, T> *, std::optional<Key> in)> default_proc;
};
...
//     return rbxx::to_opt_value<T>(data->impl->default_value);
// }
...
//         VALUE proc_ret = rb_funcall(proc, rb_intern("call"), 2, hsh, rbxx::to_opt_value<Key>(key));
...
VALUE at(VALUE self, VALUE key)
{
    using impl_t = impl<Key, T>;
    using map_t = map<Key, T>;
    using OptKey_t = std::optional<Key>;
    using OptValue_t = std::optional<T>;
    impl_t * data;
	TypedData_Get_Struct(self, impl_t, &map_t::rb_map_t, data);
    
    OptKey_t cxx_key = rbxx::from_opt_value<Key>(key);
    try
    {
        return rbxx::to_opt_value<T>(data->impl->at(cxx_key));
    } catch (const std::out_of_range & err)
    {
        ...
        {
            return Qnil;
            // return rbxx::to_opt_value<T>(data->impl->default_proc(data->impl, cxx_key));
        } else 
        {
            return Qnil;
        }
...
VALUE set_at(VALUE self, VALUE key, VALUE val)
{
    ...
    std::optional<Key> cxx_key = rbxx::from_opt_value<Key>(key);
    std::optional<T> cxx_val = rbxx::from_opt_value<T>(val);
...
//     {
// 	    rb_yield(rb_assoc_new(rbxx::to_opt_value<Key>(pair.first), rbxx::to_opt_value<T>(pair.second)));
//     }
...
//     for (const std::pair<Key, T> & pair : *data->impl)
//     {
// 	    rb_yield(rbxx::to_opt_value<Key>(pair.first));
//     }
...
//     for (const std::pair<Key, T> & pair : *data->impl)
//     {
// 	    rb_yield(rbxx::to_opt_value<T>(pair.second));
//     }
...
//     for (const std::pair<Key, T> & pair : *data->impl)
//     {
//         key = rbxx::to_opt_value<Key>(pair.first);
//         value = rbxx::to_opt_value<T>(pair.second);
...
VALUE compact_bang(VALUE self)
{
    ...
    {
        if (!elem.first.has_value() || !elem.second.has_value())
        {
            data->impl->erase(elem.first);
        }
    }
...
VALUE compact(VALUE self)
{
    ...
    impl_t * cxx_compacted;
    VALUE compacted = Qnil;
	compacted = TypedData_Make_Struct(map_t::cMap, impl_t, &map_t::rb_map_t, cxx_compacted);
    initialize<Key, T>(compacted);

    *cxx_compacted->impl = *data->impl;

    compact_bang<Key, T>(compacted);
...
//         {
//             return rbxx::to_opt_value<Key>(elem.first);
//         }
//     }
...
//     for(const auto & elem : *data->impl)
//     {
//         key = rbxx::to_opt_value<Key>(elem.first);
//         val = rbxx::to_opt_value<T>(elem.second);
...
//     return rbxx::to_opt_value<int>(data->impl->size());
// }
...
//     return rbxx::to_opt_value<bool>(std::equal(slf_beg, slf_end, cmp_beg));
// }
...
//     return rbxx::to_opt_value<bool>(std::includes(cmp_beg, cmp_end, slf_beg, slf_end));
// }
...
//     return rbxx::to_opt_value<bool>(std::includes(slf_beg, slf_end, cmp_beg, cmp_end));
// }
...
//         if (elem.first == cxx_key)
//         {
//             return rbxx::to_opt_value<T>(elem.second);
//         }
//     }
...
//     {
//         key = rbxx::to_opt_value<Key>(elem.first);
//         value = rbxx::to_opt_value<T>(elem.second);
...
void define(const std::string & cls_name)
{
    ...
    rb_define_method(map<Key, T>::cMap, "compact", (rb_func) compact<Key, T>, 0);
    rb_define_method(map<Key, T>::cMap, "compact!", (rb_func) compact_bang<Key, T>, 0);
    ...
    // rb_define_method(map<Key, T>::cMap, "filter", (rb_func) select<Key, T>, 0);
    // rb_define_method(map<Key, T>::cMap, "filter!", (rb_func) select_bang<Key, T>, 0);
